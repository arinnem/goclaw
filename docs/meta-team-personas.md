...
### Task Cascading (Multi-Team Fan-Out)

Một meta-task có thể fan-out thành **N sub-tasks** trên **M teams**. Meta-task chỉ hoàn thành khi **TẤT CẢ** sub-tasks hoàn thành.

```
Meta-Task: "Build and launch landing page" (Thiên Long Hội)
  │
  ├── Sub-task 1: "Implement UI" → Thiếu Lâm Tự
  │     └── Phong Thanh Dương delegates to Quách Tĩnh, Hoàng Sam Nữ Tử
  │     └── ✅ Completed → link status = completed
  │
  ├── Sub-task 2: "QA & deploy" → Võ Đang
  │     └── Trương Tam Phong delegates to Tiểu Long Nữ
  │     └── ✅ Completed → link status = completed
  │
  └── Sub-task 3: "Marketing launch" → Cái Bang
        └── Kiều Phong delegates to Vi Tiểu Bảo
        └── ✅ Completed → ALL done → META-TASK AUTO-COMPLETES ✅
```

Tracked via `meta_task_links` table (1:N relationship — xem chi tiết trong `meta-team-orchestrator-plan.md`).
...
### Phase 5: Task Cascading Hook (Go backend)

3 bus subscribers (chi tiết trong `meta-team-orchestrator-plan.md`):
1. **Cascade on assign** — meta-task assigned to lead → create sub-task in lead's team
2. **Aggregate completion** — all sub-tasks done → auto-complete meta-task
3. **Propagate failure** — sub-task fails → update meta-task
